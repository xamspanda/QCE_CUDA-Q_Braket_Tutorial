...
def construct_slater(walker):
    ...
    complement = np.zeros((num_qubits, num_qubits - num_particles), dtype=np.complex128)
    complement[0,0] = walker[2,0]
    complement[2,0] = -walker[0,0]
    complement[1,1] = walker[3,1]
    complement[3,1] = -walker[1,1]
...
def circuit_first_half(phi: np.ndarray):
    ...
    #complement = np.ones((num_qubits, num_qubits-num_particles))
    #complement = np.zeros((num_qubits, num_qubits - num_particles), dtype=np.complex128)
    #complement[0,0] = phi[2,0]
    #complement[2,0] = -phi[0,0]
    #complement[1,1] = phi[3,1]
    #complement[3,1] = -phi[1,1]
