...
def chemistry_preparation(mol: Mole, hf: RHF, active_orbitals=None, nel=None):
    ...
    for i in L_gamma:
        if np.count_nonzero(np.round(i - np.diag(np.diagonal(i)), 7)) != 0:
            ...
            # This only works for hydrogen case at this moment
            L_matrix = Matrix(np.round(i, 8))
            eigen = L_matrix.eigenvects()
            eigen = sorted(eigen, key=lambda x: x[0])
            
            eigval, eigvec = [], []
            for item in eigen:
                eigenvalue, _, eigenvector = item
                for vec in eigenvector:
                    vec_np = np.array([v[0] for v in vec.tolist()], dtype=float)
                    eigvec.append(vec_np)
                    eigval.append(np.array(eigenvalue, dtype=np.complex128))
            
            lambda_l.append(np.array(eigval))
            U_l.append(np.array(eigvec).T)
        else:
...
def R_basis_change(phi):
    ...
    #R[:, num_electrons//2:] = np.random.rand(num_qubits//2, num_qubits//2 - num_electrons//2)
    
    # the following only works for hydrogen, needs a way to generalize it to larger systems
    R[0, 1] = R[1, 0]
    R[1, 1] = -R[0, 0]
